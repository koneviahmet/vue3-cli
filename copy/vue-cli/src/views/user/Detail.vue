  ...
  <div class="py-6 px-4 bg-base-200 dark:bg-neutral min-h-screen">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
      ...
      <div class="mt-4 md:mt-0 flex space-x-2">
        <router-link :to="`/user/update/${route.params.id}`" class="btn btn-secondary btn-sm">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
            <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
          </svg>
          Edit User
        </router-link>
        <router-link to="/user/list" class="btn btn-outline btn-sm">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
    ...
    <div v-else-if="usersData" class="card bg-base-100 dark:bg-neutral-800 shadow-xl overflow-hidden">
      ...
      <div class="bg-gradient-to-r from-primary to-primary-focus p-8 text-center">
        ...
        <h2 class="text-2xl font-bold text-primary-content mt-4">{{ usersData.name }} {{ usersData.lastName }}</h2>
      ...
      <div class="card-body p-6 bg-base-100 dark:bg-neutral-800 text-base-content dark:text-neutral-content">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="space-y-4">
            <div class="card bg-base-200 dark:bg-neutral-700 shadow-sm p-4">
              ...
              <p class="text-lg font-medium">{{ usersData.name }} {{ usersData.lastName }}</p>
            </div>
        ...
        <div class="card-actions justify-end mt-8">
          <router-link :to="`/user/update/${usersData.id}`" class="btn btn-primary">
    ...
    <div v-else class="card bg-base-100 dark:bg-neutral-800 shadow-xl p-8 text-center">
      ...
      <router-link to="/user/list" class="btn btn-primary mx-auto">
        Go to User List
      </router-link>
    </div>
  </div>
...
const getUserInitials = () => {
  ...
  return (usersData.value.name.charAt(0) + (usersData.value.lastName ? usersData.value.lastName.charAt(0) : '')).toUpperCase();
};
