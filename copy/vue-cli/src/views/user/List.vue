  ...
  <div class="py-6 px-4 min-h-screen">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
      ...
      <div class="mt-4 md:mt-0">
        <router-link to="/user/signIn" class="btn btn-primary">
            ...
            <tr v-for="user in filteredUsers" :key="user.id" class="hover:bg-base-200">
              ...
              <td class="py-4 px-4">
                <div class="flex items-center">
                  ...
                  <div class="ml-4">
                    <div class="font-medium text-base-content">{{ user.name }} {{ user.lastName }}</div>
...
const filteredUsers = computed(() => {
  ...
  return usersData.value.filter(user => {
    ...
    if (searchQuery.value) {
      ...
      const nameMatch = (user.name + " " + user.lastName).toLowerCase().includes(query);
...
const getUserInitials = (user) => {
  if (!user.name) return '?';
  return (user.name.charAt(0) + (user.lastName ? user.lastName.charAt(0) : '')).toUpperCase();
};
