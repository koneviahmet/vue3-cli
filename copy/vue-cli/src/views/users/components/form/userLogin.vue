    ...
    <div class="card-body p-6">
      ...
      <div class="form-control">
        ...
        <div class="flex justify-between items-center mt-2">
          ...
          <router-link to="/users/forgot-password" class="text-sm text-primary hover:underline">Forgot password?</router-link>
        </div>
      </div>
      ...
      <div class="text-center">
        ...
        <router-link to="/users/signIn" class="text-primary hover:underline font-medium">
          Create new account
        </router-link>
      </div>
    </div>
        ...
        .then((validateSuccess) => {
          ...
          if (validateSuccess.valid) {
            loginUser({ ...values }).then(
              (response) =>            
                {                  
                  response.id && router.push(`/users/detail/${response.id}`)
                }
            );
          }
        })
