...
<div v-if="model == 'basic_close' && aShow" :class="getClass" role="alert">
  ...
  <button @click="setShow" type="button" class="ml-auto -mx-1.5 -my-1.5  text-blue-500 rounded-lg focus:ring-2 focus:ring-blue-400 p-1.5 hover:bg-blue-200 inline-flex h-8 w-8">
  ...
  export default {
    ...
    setup(props) {
      ...
      const colorArr = {
          'basic':{        
            ...
            'yellow':{
              ...
              "outline":'bg-white border-2 border-yellow-100 rounded-lg  text-yellow-500'
            },
            'green':{
              "fill":'bg-green-100 rounded-lg  text-green-700',
              "outline":'bg-white border-2 border-green-100 rounded-lg  text-green-500'
            },
            'blue':{
              "fill":'bg-blue-100 rounded-lg  text-blue-700',
              "outline":'bg-white border-2 border-blue-100 rounded-lg  text-blue-500'
            },
            'indigo':{
              "fill":'bg-indigo-100 rounded-lg  text-indigo-700',
              "outline":'bg-white border-2 border-indigo-100 rounded-lg  text-indigo-500'
            },
            'purple':{
              "fill":'bg-purple-100 rounded-lg  text-purple-700',
              "outline":'bg-white border-2 border-purple-100 rounded-lg  text-purple-500'
            },
            'pink':{
              "fill":'bg-pink-100 rounded-lg  text-pink-700',
              "outline":'bg-white border-2 border-pink-100 rounded-lg  text-pink-500'
            }
        },
        'icon':{        
            'gray':{
      ...
      const getClass = computed(() => {
        return colorArr[props.model][props.color][props.type] + " " +  sizeArr[props.size];
      })

      const setShow = () => {aShow.value = false}
