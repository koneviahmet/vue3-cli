  ...
  <div class="m-4">
    <form class="w-full max-w-sm">
      ...
      <div class="md:flex md:items-center">
        ...
        <div class="md:w-2/3">
          <button @click="login" class="shadow mr-3 bg-purple-500 hover:bg-purple-400 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded" type="button">
            Login
          </button>
        </div>
      </div>
        ...
        const login = () => {
          ...
          validate().then(validateSuccess => {
            ...
            if(validateSuccess.valid){
                loginUser({id:1})
                  .then(response => response.id && router.push(`/user/detail/${response.id}`))
            }
